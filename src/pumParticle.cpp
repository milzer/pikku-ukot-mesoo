...
inline void CPumParticle::Create(float x, float y, float i, float j, float accel, int lt, const BYTE col, int fl, CPumSysPointers *sp)
{
    if (y < 1.0f) return;
    if (y > 198.0f) return;
    if (x < 1.0f) return;
...
inline int CPumParticle::Update(float frameTime)
{
    ...
    if (m_Flags & PRT_GRAVITY) m_Vec.y += m_SP->Options->Gravity * frameTime;

    if (m_Flags & PRT_XFRICTION) m_Vec.x *= m_Acceleration;
    if (m_Flags & PRT_YFRICTION) m_Vec.y *= m_Acceleration;
    ...
    if (m_Flags & PRT_GIB) {
        ...
        if (!(rand()&3))
            SpawnParticles(_PRT_BLOOD, m_fXpos, m_fYpos, (float)m_Counter, 2.0f, (float)m_Misc*TimeFactor, (float)m_Misc*TimeFactor, 1.0f, 1, 0, 0, m_SP);
    }

    if (m_Flags & PRT_DEBRIS) {
        if (!(rand()&3))
            SpawnParticles(_PRT_SPARK, m_fXpos, m_fYpos, (float)(rand()&255), 0.0f, 0.0f, 0.5f*TimeFactor, 1.0f, 1, 0, 0, m_SP);
    }
                    ...
                    int num, int lifetime, BYTE col, CPumSysPointers *sp)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        switch (type) {
        case _PRT_BLOOD:
            sp->Particle->Add(x, y, cosinus[_angle] * _power, sinus[_angle] * _power, 0.992f, 1000.0f, 213 + (rand()&3), _PRT_BLOOD);
            break;
        case _PRT_GIB:
            sp->Particle->Add(x, y, cosinus[_angle] * _power, sinus[_angle] * _power, 0.999f, 4000.0f, col, _PRT_GIB);
            break;
        case _PRT_SPARK:
            sp->Particle->Add(x, y, cosinus[_angle] * _power, sinus[_angle] * _power, 0.9f, (float)(145 + (rand()&63)), (char)205, _PRT_SPARK);
            break;
        case _PRT_FIREEMIT:
