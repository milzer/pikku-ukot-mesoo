...
void CPumPlayer::UpdateControls(DWORD* pSrf)
{
    ...
    if (m_Vec.x > -m_WalkSpeed && m_KeyBuffer[PLR_C_LEFT] == TRUE && m_KeyBuffer[PLR_C_RIGHT] == FALSE)
    {
        if (m_Frame > 1)
        {
            if (m_iAngle < 64)
            {
                m_iAngle = 128 - m_iAngle;
            }
            else
            {
                m_iAngle = 192 - (m_iAngle - 192);
            }
        }
    ...
    {
        SpawnParticles(_PRT_FASTSPARK, cosinus[m_iAngle] * 2 + m_iXpos + (m_Vec.x * 0.02f) + 1,
                       sinus[m_iAngle] * 2 + m_iYpos + (m_Vec.y * 0.02f) - 1, (float)m_iAngle, 14, 0.8f * TimeFactor, 2.2f * TimeFactor, 0.0f,
                       4, 255, (BYTE)0, m_SP);
        SpawnParticles(_PRT_FASTSPARK, cosinus[m_iAngle] * 3 + m_iXpos + (m_Vec.x * 0.02f) + 1,
                       sinus[m_iAngle] * 3 + m_iYpos + (m_Vec.y * 0.02f) - 1, (float)m_iAngle + 40, 4, 0.2f * TimeFactor, 0.9f * TimeFactor,
                       0.0f, 4, 255, (BYTE)0, m_SP);
        SpawnParticles(_PRT_FASTSPARK, cosinus[m_iAngle] * 3 + m_iXpos + (m_Vec.x * 0.02f) + 1,
                       sinus[m_iAngle] * 3 + m_iYpos + (m_Vec.y * 0.02f) - 1, (float)m_iAngle + 216, 4, 0.2f * TimeFactor, 0.9f * TimeFactor,
                       0.0f, 4, 255, (BYTE)0, m_SP);
...
void CPumPlayer::Update(DWORD* pSrf, float frameTime)
{
    ...
    if (m_Health < 0)
    {
        ...
        SpawnParticles(_PRT_GIB, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.0f, (-m_Health) / 6 * TimeFactor, 0, (-m_Health) / 5, 0,
                       195, m_SP);
        SpawnParticles(_PRT_BLOOD, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.0f, (-m_Health) / 16 * TimeFactor, 0, (-m_Health) * 12, 0,
                       195, m_SP);
    ...
    if (CheckBHit(0, m_Vec.y * frameTime, pSrf))
    {
        ...
        {
            SpawnParticles(_PRT_BLOOD, m_fXpos, m_fYpos + 1, 0, 256.0f, 0, (float)(m_Vec.y - (5.5 * TimeFactor)), 1.0f,
                           RoundDef((m_Vec.y * frameTime - 2) * 32), 0, 0, m_SP);
...
void CPumPlayer::Draw(DWORD* pSrf)
{
    ...
    for (y = 0; y < 4; y++)
    {
        ...
        {
            if (*gptr)
                if ((m_iXpos + x - 2) >= 0 && (m_iXpos + x - 2) < ScreenWidth && (m_iYpos + y - 1) >= 0
                        && (m_iYpos + y - 1) < ScreenHeight)
                {
...
{
    SpawnParticles(_PRT_BLOOD, m_fXpos, m_fYpos, 40.74366f * ((float)atan2(v->y, v->x) + 3.14159f), 32, 0,
                   -d * TimeFactor / 10, 0, d * 2, 0, 0, m_SP);
    ...
    if (m_AccuracyMod > 4.0f)
    {
        m_AccuracyMod = 4.0f;
    }
    if (m_WalkSpeed < 0.5f * TimeFactor)
    {
        m_WalkSpeed = 0.5f * TimeFactor;
    }
    if (m_JumpPower < 0.9f * TimeFactor)
    {
        m_JumpPower = 0.9f * TimeFactor;
    }
}
