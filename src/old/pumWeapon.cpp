...
void CPumBullet::Activate(float frameTime)
{
    ...
    {
        SpawnParticles(_PRT_FIREEMIT, m_fXpos - (m_Vec.x / TimeFactor), m_fYpos - (m_Vec.y / TimeFactor), 192.0f, 100.0f, 0.1f,
                       TimeFactor, 0.0f, (int)m_Blow * 2, 300 + RandInt(0, 200), 0, m_SP);
    }
    else if (m_Blow > 0.0f)
    {
        ...
        for (l = 1.8f; l < 20.0f; l += 6.0f)
            if (m_Blow > l)
            {
                m_SP->Sound->PlaySample(1, 7, m_iXpos);
            }

        SpawnParticles(_PRT_EXPLOSION, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.1f, m_Blow * TimeFactor, 0, (int)m_Blow * 10, 255, 0,
                       m_SP);
        SpawnParticles(_PRT_FASTSPARK, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.1f, m_Blow * TimeFactor, 0, (int)m_Blow * 5, 255, 0,
                       m_SP);
        SpawnParticles(_PRT_SPARK, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.1f, m_Blow * 0.7f * TimeFactor, 0, (int)m_Blow * 2, 255,
                       0, m_SP);
        if (m_Blow > 1.9)
        {
            SpawnParticles(_PRT_DEBRIS, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.1f, m_Blow * 0.6f * TimeFactor, 0, (int)m_Blow * 2, 255,
                           0, m_SP);
        }
...
int CPumBullet::Update(float frameTime)
{
    ...
    */
    ...
    {
        SpawnParticles(_PRT_SPARK, m_fXpos, m_fYpos, Angle(m_Vec.x, m_Vec.y, 0, 0), 32.0f, 0.5f * TimeFactor,
                       1.1f * TimeFactor, 0, rand() & 3, 255, 0, m_SP);
    }
    else if (m_Flags & WPN_EFF_SPARKS)
    {
        SpawnParticles(_PRT_SPARK, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.5f * TimeFactor, 1.1f * TimeFactor, 0, rand() & 1, 255, 0,
                       m_SP);
    }
    ...
    if (m_Onscreen)
    {
        if (pix)
        {
            if (!(rand() & 3))
            {
                m_SP->Sound->PlaySample(m_SoundWallId[0], m_SoundWallId[1], m_iXpos);
            }
            ...
            if (m_Flags & WPN_EFF_WALLSPARK)
            {
                ...
                if (!(rand() & 7))
                {
                    SpawnParticles(_PRT_SPARK, m_fXpos, m_fYpos, 0.0f, 256.0f, 0.1f, 0.7f * TimeFactor, 0, 1, 255, 0, m_SP);
                }
            }
