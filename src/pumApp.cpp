...
int CpumApp::InitGame(SDE_PUMOPTIONS *opt)
{
    ...
    if (m_SoundSystem) delete m_SoundSystem;
    ...
    if (m_WeaponSystem) delete m_WeaponSystem;
...
void CpumApp::OnFlip()
{
    ...
    case eMainmenu:
        ...
        if (tmp == 1) {
            ResetGame(lv);
            GameState = eGame;
        } else if (tmp == 2) GameState = eOptions;
        else if (tmp == 3) Shutdown();
        break;

    case eOptions:
        tmp = OptionsMenu(&ang);
        if (tmp == 1) GameState = eMainmenu;
        if (tmp == 2) {
...
int CpumApp::OptionsMenu(float *a)
{
    ...
    for (i = 0; i < 32; i++) {
        if (col[i] >= 0)    col[i] -= (int)(m_fFrameTime * 300);
...
void CpumApp::Blur(DWORD *ptr, int c)
{
    ...
    for (p = 0; p < 320; p++)           ptr[ p ] = 0;
    for (p = 63680; p < 64000; p++) ptr[ p ] = 0;
    for (p = 0; p < 64000; p += 320)      ptr[ p ] = 0;
    for (p = 319; p < 64000; p += 320)    ptr[ p ] = 0;
}
